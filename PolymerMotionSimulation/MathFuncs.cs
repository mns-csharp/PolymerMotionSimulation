...
namespace PolymerMotionSimulation
{
    public static class MathFuncs
    {
        ...
        public static Point2d GetRandPointAtRadius(Point2d currentLoc, double radius)
        {
            /*
            ...
            x = currentLoc.X + r * Math.Cos(theta);
            y = currentLoc.Y + r * Math.Sin(theta);
            */

            double dx = Global.Random.NextDouble();  // obtain new move distance for X axis
            dx = MathFuncs.GetNormalizeBetween(dx, -5, 5);
